...
**Repeated logins, or "CSRF session token is missing"?** If you're also
running another Flask app (e.g. `4thealth-plus`) at the same hostname —
`localhost:8100` alongside 4tExecutive's `localhost:8200`, say — check for
a session cookie name collision first. Browser cookies are scoped by
domain and path, **not port** (RFC 6265), and Flask's default session
cookie name is `session` for every app unless overridden. Two unrelated
Flask apps both using the default name at the same hostname will silently
overwrite each other's session cookie on every navigation between them,
which reads as random logouts and CSRF failures with no obvious cause.
4tExecutive sets `SESSION_COOKIE_NAME = "4texecutive_session"`
specifically to make this impossible (`app/__init__.py`) — if you're
still seeing it, check whether whatever else you're running shares a
cookie name with something else in the mix.
